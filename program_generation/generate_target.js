...
function addExpression(obj, expressions, offset, inline = false, guard = true) {
	...
	let string = "";
	let add_string1 = "";
	add_string1 += json2jsGenerateString(obj, expressions, offset+1, inline, guard);
	string += add_string1;
	add_string1 = add_string1.trim();
	return string;
}

function json2jsGenerateString(obj, expressions, offset, inline = false, guard = true) {
	...
	if (Array.isArray(obj[0])) {
		...
		for (var key in obj) {
			let add_string = "";
			add_string += json2jsGenerateString(obj[key], expressions, offset, inline, guard);
			string += add_string;
		...
		if (obj[3] == "in") {
			string += indent(offset);
			string += "for (let " + obj[1] + " in " + obj[4] + ") {\n";

			string += json2jsGenerateString(obj[2], expressions, offset+1, inline, guard);
			...
			if (guard) {
				...
				string +=
					"for (let " + obj[1] + " = " + json2jsGenerateString(obj[3], expressions, offset, true) + ", j = 0; "
								+ obj[1] + " < " + json2jsGenerateString(obj[4], expressions, offset, true) + "; "
								+ obj[1] + " += " + json2jsGenerateString(obj[5], expressions, offset, true) + ") {\n";
				string += indent(offset+1) + "j++;\n"
				string += indent(offset+1) + "if (j > 100) break;\n"

				// string +=
				// 	 "for (let " + obj[1] + " = " + json2jsGenerateString(obj[2], expressions, offset, true) + ", j = 0; "
				// 				 // + indent(offset+1) +
				// 				 + obj[1] + " < " + json2jsGenerateString(obj[3], expressions, offset, true) + "; "
				// 				 + obj[1] + " += " + json2jsGenerateString(obj[4], expressions, offset, true) + ") {\n";
				//  string += indent(offset+1) + "j++;\n"
				...
				string +=
					"for (let " + obj[1] + " = " + json2jsGenerateString(obj[3], expressions, offset, true) + "; "
								+ obj[1] + " < " + json2jsGenerateString(obj[4], expressions, offset, true) + "; "
								+ obj[1] + " += " + json2jsGenerateString(obj[5], expressions, offset, true) + ") {\n";
			}
			...
			// 	let add_string = "";
			// 	add_string += json2jsGenerateString(obj[key], expressions, offset+1, inline, guard);
			// 	string += add_string;
			// }
			string += json2jsGenerateString(obj[2], expressions, offset+1, inline, guard);
		...
		if (guard) {
			...
			string += indent(offset) + "while (" + json2jsGenerateString(obj[1], expressions, offset, true) + ") {\n";
			string += indent(offset+1) + "j++;\n"
			string += indent(offset+1) + "if (l) break;\n"
			string += indent(offset+1) + "if (j > 100) {console.log(\"*"+obj[1]+"\"); l = true; break};\n"
		} else {
	...
	} else if (obj[0] == "get_index") {

		if (!inline) string += indent(offset);
		string += obj[1] + "[";
		string += json2jsGenerateString(obj[2], expressions, offset, true);
		string += "]";
		...
		string += obj[1] + "[";
		string += json2jsGenerateString(obj[2], expressions, offset, true);
		string += "] = " + json2jsGenerateString(obj[3], expressions, offset, true) + ";\n";
		...
		if (obj[0].match(unary)) {
			...
			// binary operator
			string += json2jsGenerateString(obj[1], expressions, offset, true);
			string += " " + obj[0] + " ";
			string += json2jsGenerateString(obj[2], expressions, offset, true);
			...
			} else if (obj[0][obj[0].length-1] == "_") {
				obj[0] = obj[0].substring(0, obj[0].length-1);
				string += obj[0];
				start_args = 1;
				parentheses = false;
			...
			for (var key = start_args; key < obj.length; key++) {
				if (Array.isArray(obj[key])) {
					args_string += json2jsGenerateString(obj[key], expressions, offset, true);
				} else {
					args_string += obj[key];
				}
...
function json2js(script, expressions = [], guard = true, inline = false) {
	let obj = JSON.parse(script);
	var string = "";
	string += json2jsGenerateString(obj, expressions, 0, inline, guard);
	return string;
}
